  ...
  return (
    <Card className="modern-card-tall">
      ...
      <CardContent className="space-y-6">
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent className="glass-card max-h-[200px]">
                        {indianStates.map((state) => (
                          <SelectItem key={state} value={state} className="hover:bg-muted/50">
                            {state}
                          </SelectItem>
                        ))}
                      </SelectContent>
              ...
              <div className="flex flex-wrap gap-2">
                {prebuiltQuestions.map((question, index) => (
                  <Badge
                    ...
                    className="cursor-pointer glass-card-sm text-xs px-2 py-1 hover:bg-blue-500/20 transition-colors"
        ...
        {isLoading && (
          <div className="glass-card-sm p-4 flex items-center justify-center">
            <Loader2 className="mr-2 h-6 w-6 animate-spin text-blue-600 dark:text-blue-400" />
            <p className="text-muted-foreground">{t('fetching_summary')}...</p>
          </div>
        )}
          ...
          <div className="space-y-4 pt-4 border-t border-border/50">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">{t('ai_summary_title')}</h3>
              </div>
              {result.audio && (
                <Button 
                  variant="outline" 
